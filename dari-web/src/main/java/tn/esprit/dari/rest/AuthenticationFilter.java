...
public class AuthenticationFilter implements ContainerRequestFilter  {
	...
	//verifier le header 
	//authorization : ou on va envoyer le token
		// verifier si l'authentification est basé sur les tokens
		// verifier que authenfication n'est pas null et commence par le bearer
		// valider le token
		// bearer : shema d'authenfication : type d'authentifcation securé
		//voir la classe EmployeResource
		...
		public void filter(ContainerRequestContext requestContext) throws IOException {
			...
			try {
				...
				Claims claims =  Jwts.parser().setSigningKey("simplekey".getBytes("UTF-8")).parseClaimsJws(token).getBody();
	            if( method != null){
	                ...
	                if(permission != Role.ROLE_a0 ) {
	                    // Get Role from jwt
	                    String roles = claims.get("aud", String.class);
